...
def uint64sToUint256 (x1 x2 x3 x4: UInt64): Fin 115792089237316195423570985008687907853269984665640564039457584007913129639936 :=
  ((Fin.ofNat (x1.toNat) * 18446744073709551616 + Fin.ofNat (x2.toNat)) * 18446744073709551616 + Fin.ofNat (x3.toNat)) * 18446744073709551616 + Fin.ofNat (x4.toNat)
...
def parseMemoryMapTailRec (bin: List UInt8) (acc: List (UInt64 × (Fin 115792089237316195423570985008687907853269984665640564039457584007913129639936))): List (UInt64 × (Fin 115792089237316195423570985008687907853269984665640564039457584007913129639936)) :=
  match bin with
  | [] => acc
...
def main : IO Unit := do
  let compilationRes ← IO.Process.run {cmd:="cairo-compile", args:=#["../cairo-vm-go/integration_tests/cairo_files/factorial.cairo", "--proof_mode", "--output", "./factorial_compiled.json"]}
  -- IO.println compilationRes
  let runRes ← IO.Process.run {cmd:="sh", args:=#["-c", "../cairo-vm-go/bin/cairo-vm run  --proofmode --tracefile ./factorial_trace --memoryfile ./factorial_memory ./factorial_compiled.json"]}
  -- IO.println runRes
